...
export function ClubStructureShowcase({
  ...
  return (
    <div className="grid grid-cols-1 gap-px bg-white/8 lg:grid-cols-[0.4fr_0.6fr]">
      ...
      <section className="bg-[var(--color-ink)] p-4 sm:p-6 lg:p-8">
        ...
        <ul className="mt-8 grid grid-cols-1 gap-px bg-white/8 sm:grid-cols-2 xl:grid-cols-3">
          {teamCategories.items.map((category) => (
            <li key={category.name} className="bg-[var(--color-ink)]">
              <article className="h-full">
                <div className="site-grid relative flex aspect-[16/10] items-start overflow-hidden rounded-lg bg-[var(--color-gold)] p-4 text-[var(--color-ink)]">
                ...
                <footer className="flex min-h-20 items-center justify-between gap-3 p-4">
                  ...
                  {category.status === "coming-soon" ? (
                    <span className="shrink-0 rounded-md px-2.5 py-1 font-condensed text-[0.6rem] font-bold uppercase tracking-[0.16em] text-[var(--color-gold)] bg-[var(--color-gold)]/10">
